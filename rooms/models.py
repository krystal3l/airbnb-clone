...
class Room(core_models.TimeStampModel):
    ...
    # related_name : target을 위한 이름
    # user(target)로 등록된 모든 rooms 조회. name을 지정하지 않으면 room_set.
    host = models.ForeignKey(
        "users.User", related_name="rooms", on_delete=models.CASCADE
    )
    # room_type의 데이터를 삭제하더라도 room은 지워지지 않고 room_type이 null로 유지됨.
    # roomtype이 '호텔룸'인 rooms 조회해라.
    ...
    def get_absolute_url(self):
        # reverse() : url name을 필요로 하는 function, url을 return함.
